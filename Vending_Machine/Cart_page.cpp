...
public:
            ...
            if (jshandler.contains(product_id))
            {
                if (jshandler.supply_manager(product_id, -1))
                {
                    if (user.get_user_balance() - jshandler.find<Product>(product_id).price >= 0)
                    {
                        ...
                        std::cout<<(*(user.products.end()-1)).get_name()<<" added to your cart..."<<std::endl;
                    }
                    else
    ...
    static bool cart_page(User &user)
    {
        ...
        for (auto i : Map)
            std::cout << std::setw(20) <<std::right<< i.first << "   x" << i.second << std::endl;
            std::cout << std::setw(100) << std::setfill('-') << "" << std::endl;
        }
        bool cart_flag = true;
        while (cart_flag)
        {
            ...
            try
            {
                ...
                case 2:
                {
                     if(user.products.empty())
                     {
                        std::cout<<"No products to delete..."<<std::endl;
                        break;
                     }
